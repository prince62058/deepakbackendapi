...
const updatePlayTimeStamp = async (req, res) => {
  ...
  try {
    ...
    if (!watchHistory) {
      ...
      console.log(
        `✅ Created new watch history record with timestamp: ${normalizedPlayTime}s`,
      );
    } else {
      ...
      console.log(
        `✅ Updated watch history: ${oldValue}s → ${normalizedPlayTime}s`,
      );
    }
    ...
    if (activePlan) {
      ...
      // 🛡️ ANTI-EXPLOIT: Check for large time jumps (seeking)
      ...
      if (incrementalMinutes > 0 && isSecondsReasonable) {
        ...
        console.log(
          `⚠️ Seek detected: ${incrementalSeconds.toFixed(1)}s jump. No reward given.`,
        );
      }
      ...
      if (billableMinutes > 0 && rewardAmount > 0) {
        ...
        await Transaction.create({
          ...
          Type: "CREDIT",
          status: "APPROVED",
          message: `Watch & Earn reward for ${billableMinutes.toFixed(2)} min at ₹${effectiveRewardRate}/min`,
        });
